              ...
              >
                ...
                {selectedFactor?.name === f.name && (
                  <div className="mt-2 space-y-1">
                    <p className="text-xs text-muted-foreground italic">{FACTOR_DESCRIPTIONS[f.name] ?? "Assesses your financial health across this dimension."}</p>
                    <p className="text-sm text-muted-foreground leading-relaxed">{f.explanation}{f.value ? ` (${f.value})` : ""}</p>
                  </div>
                )}
              </button>
            ))}
