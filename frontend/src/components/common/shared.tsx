...
export function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border px-8 py-12 text-center">
      {Icon && (
        <div className="mb-3 rounded-full bg-surface-container-high p-3">
          <Icon className="h-6 w-6 text-text-muted" />
        </div>
      )}
      <h3 className="text-sm font-medium text-text-primary">{title}</h3>
      {description && <p className="mt-2 max-w-xs text-xs text-text-muted leading-relaxed">{description}</p>}
